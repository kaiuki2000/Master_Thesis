...
\begin{algorithm}[H]
   ...
   \State 1. Define $U_C$ and $U_M$ as for usual \acrshort{qaoa}
   \State 2. Create the variational quantum circuit, using a \texttt{@qml.qnode}, based on the \acrshort{qaoa} ansatz (Layering $U_C$ and $U_M$ $p$ times, for $p = \texttt{n\_layers}$ layers). Note that we start with \texttt{qml.Hadamard} on all wires
   \State 3. The objective function is defined as the expectation value of the problem Hamiltonian, obtained from the previously defined \texttt{@qml.qnode}, through \texttt{qml.expval(H\_C)}
